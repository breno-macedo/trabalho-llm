...
```
No computador, editar essas duas variáveis no topo de **`clean_file.py`** e
...
> Para reproduzir o artigo (52 atributos), usar o caminho `clean_file.py` + `aggregate.py`.
...
8. **Revisão pelo LLM** — o LLM lê `review_events.md` e produz um veredito por alerta em
   `llm_verdicts.json`. No nosso caso, o Claude fez isso; o arquivo está em
